...
the tests on an Ubuntu 16.04 machine and had fewer failures. The two executables appeared to
behave differently. For example, with the Windows version, the AverageTime statistic was never
updated (it was always 0), but with the Linux version, it did - even though it was too high.
Both were failures in this case. However, the handling of in-flight requests appeared to fail
only on Windows and not Linux.

The bugs I found were:
1. It takes about 5 seconds to return a job ID when submitting a password to /hash. The
   specification says it should return immediately. The server then hashes the password
   immediately after returning the job ID. The specification says it should wait 5
   seconds after returning the job ID.

2. Nondeterministic behavior when handling concurrent requests - either the wrong job IDs
...
3. Average time statistic not being updated on Windows. On Linux, it was being updated but
   was far higher than it should have been (exceeding the time of the longest-running request).

4. On Windows, some connections were forcibly closed when the shutdown command was issued.
...
5. The server accepted new password requests after the shutdown signal was given. The validity of
   this test was reinforced by the server itself printing the message "recieved request after shutdown".
   The specification says it should reject any new requests after the shutdown signal.
